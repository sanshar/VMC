...
#ifdef Relativistic
using CorrSampleFunctorr = boost::function<void (std::vector<Eigen::VectorXcd>&, std::vector<std::complex<double>>&)>; 
using functor1r = boost::function<std::complex<double> (VectorXcd&, VectorXcd&, std::complex<double>&, std::complex<double>&, std::complex<double>&)>;
using functor2r = boost::function<void (VectorXcd&, VectorXcd&, VectorXcd&, DirectMetric&, std::complex<double>&, std::complex<double>&, std::complex<double>&)>;
using functor5r = boost::function<std::complex<double> (VectorXcd&, VectorXcd&, MatrixXcd&, MatrixXcd&, std::complex<double>&, std::complex<double>&, std::complex<double>&)>;
using functor6r = boost::function<std::complex<double> (VectorXcd&, VectorXcd&, DirectLM&, std::complex<double>&, std::complex<double>&, std::complex<double>&)>;
...
void runRelVMC(Wave& wave, Walker& walk) {
  ...
  getGradientWrapper<Wave, Walker> wrapper(wave, walk, schd.stochasticIter, schd.ctmc);
  functor1r getStochasticGradient = boost::bind(&getGradientWrapper<Wave, Walker>::getGradient, &wrapper, _1, _2, _3, _4, _5, schd.deterministic);
  functor2r getStochasticGradientMetric = boost::bind(&getGradientWrapper<Wave, Walker>::getMetric, &wrapper, _1, _2, _3, _4, _5, _6, _7, schd.deterministic);
  functor5r getStochasticGradientHessian = boost::bind(&getGradientWrapper<Wave, Walker>::getHessian, &wrapper, _1, _2, _3, _4, _5, _6, _7, schd.deterministic);
  functor6r getStochasticGradientHessianDirect = boost::bind(&getGradientWrapper<Wave, Walker>::getHessianDirect, &wrapper, _1, _2, _3, _4, _5, _6, schd.deterministic);

  CorrSampleWrapper<Wave, Walker> wrap(schd.CorrSampleFrac * schd.stochasticIter);
  CorrSampleFunctorr runCorrelatedSampling = boost::bind(&CorrSampleWrapper<Wave, Walker>::run, &wrap, _1, _2);
  ...
  if (schd.method == amsgrad || schd.method == amsgrad_sgd) {
      if (schd.stepsizes.empty()) {
        ...
        relAMSGrad optimizer(schd.stepsizes, schd.decay1, schd.decay2, schd.maxIter, schd.avgIter);
        optimizer.optimize(vars, getStochasticGradient, runCorrelatedSampling, schd.restart); 
      }
  }
  else {
