...
struct Walker<Corr, relSlater> {
  ...
  Walker(const Corr &corr, const relSlater &ref, const Determinant &pd) : d(pd), refHelper(ref, pd), corrHelper(corr, pd) {}; 
  ...
  void OverlapWithOrbGradient(const relSlater &ref, Eigen::VectorXcd &grad, std::complex<double> detovlp) const
  {
    int norbs = Determinant::norbs;
  ...
  void OverlapWithOrbGradientGhf(const relSlater &ref, Eigen::VectorXcd &grad, std::complex<double> detovlp) const
  {
    int norbs = Determinant::norbs;
    Determinant walkerDet = d;
  ...
  void OverlapWithGradient(const relSlater &ref, Eigen::VectorBlock<VectorXcd> &grad) const
  {
    ...
    if (ref.determinants.size() <= 1 && schd.optimizeOrbs) {
      ...
      VectorXcd gradOrbitals;
      if (ref.hftype == UnRestricted) {
        gradOrbitals = VectorXcd::Zero(4 * ref.HforbsA.rows() * ref.HforbsA.rows());
        OverlapWithOrbGradient(ref, gradOrbitals, detovlp);
      }
      else {
        gradOrbitals = VectorXcd::Zero(2 * ref.HforbsA.rows() * ref.HforbsA.rows());
  ...
  void OverlapWithLocalEnergyGradient(const Corr &corr, const relSlater &ref, workingArray &work, Eigen::VectorXd &gradEloc) const
  {
