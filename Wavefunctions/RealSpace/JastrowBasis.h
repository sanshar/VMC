...
//s-orbital Number Counter
  ...
  void AB_eval_deriv(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad)
  {
    ...
    for (int I = 0; I < norbs; I++)
    {
        ...
        //gradx
        gradxNum[I] += aoValues[1*norbs + I];
        gradDenom[0] += aoValues[1*norbs + I];
        //grady
        gradyNum[I] += aoValues[2*norbs + I];
        gradDenom[1] += aoValues[2*norbs + I];
        //gradz
        gradzNum[I] += aoValues[3*norbs + I];
        gradDenom[2] += aoValues[3*norbs + I];
    }
    values = num / denom;
    grad[0] = (gradxNum * denom - num * gradDenom[0]) / (denom * denom);
    grad[1] = (gradyNum * denom - num * gradDenom[1]) / (denom * denom);
  ...
  void AB_eval_deriv2(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad, array<VectorXd, 3> &grad2)
  {
    ...
    for (int I = 0; I < norbs; I++)
    {
        ...
        gradxNum[I] += aoValues[1*norbs + I];
        gradDenom[0] += aoValues[1*norbs + I];
        gradxxNum[I] += aoValues[4*norbs + I];
        grad2Denom[0] += aoValues[4*norbs + I];

        //grady
        gradyNum[I] += aoValues[2*norbs + I];
        gradDenom[1] += aoValues[2*norbs + I];
        gradyyNum[I] += aoValues[7*norbs + I];
        grad2Denom[1] += aoValues[7*norbs + I];

        //gradz
        gradzNum[I] += aoValues[3*norbs + I];
        gradDenom[2] += aoValues[3*norbs + I];
        gradzzNum[I] += aoValues[9*norbs + I];
        grad2Denom[2] += aoValues[9*norbs + I];
    }
    values = num / denom;
    grad[0] = (gradxNum * denom - num * gradDenom[0]) / (denom * denom);
    grad[1] = (gradyNum * denom - num * gradDenom[1]) / (denom * denom);
...
//Grid of Gaussians, read from file
  ...
  void mG_eval(int elec, const vector<Vector3d> &x, VectorXd &values)
  {
    const VectorXd &relec = x[elec];
    int ngrid = schd.gridGaussians.size();
    ...
    double denom = 0.0;
    double oldsigma = 1.0;
    Vector3d oldrI = Vector3d::Constant(1.0e6);
    double oldval = 0.0;
    for (int I = 0; I < ngrid; I++)
    {
      double sigma = schd.gridGaussians[I].first;
  ...
  void mG_eval_deriv(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad)
  {
    const VectorXd &relec = x[elec];
    int ngrid = schd.gridGaussians.size();
    ...
    Vector3d oldrI = Vector3d::Constant(1.0e6);
    double oldval = 0.0;
    Vector3d oldg = Vector3d::Constant(1.0e6);
    for (int I = 0; I < ngrid; I++)
    {
      double sigma = schd.gridGaussians[I].first;
      Vector3d &rI = schd.gridGaussians[I].second;
      ...
      if ((rI - oldrI).squaredNorm() == 0.0)
      {
        ...
        denom += (val - oldval * ovlp);
        //gradx
        gradxNum[I] += (g(0) - oldg(0) * ovlp);
        gradDenom[0] += (g(0) - oldg(0) * ovlp);
        //grady
        gradyNum[I] += (g(1) - oldg(1) * ovlp);
        gradDenom[1] += (g(1) - oldg(1) * ovlp);
        //gradz
        gradzNum[I] += (g(2) - oldg(2) * ovlp);
        gradDenom[2] += (g(2) - oldg(2) * ovlp);
      }
      else
      {
        num[I] += val;
  ...
  void mG_eval_deriv2(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad, array<VectorXd, 3> &grad2)
  {
    const VectorXd &relec = x[elec];
    int ngrid = schd.gridGaussians.size();
    ...
    double oldval = 0.0;
    Vector3d oldg = Vector3d::Constant(1.0e6);
    Vector3d oldg2 = Vector3d::Constant(1.0e6);
    for (int I = 0; I < ngrid; I++)
    {
      double sigma = schd.gridGaussians[I].first;
      Vector3d &rI = schd.gridGaussians[I].second;
      ...
      if ((rI - oldrI).squaredNorm() == 0.0)
      {
        ...
        gradxNum[I] += (g(0) - oldg(0) * ovlp);
        gradDenom[0] += (g(0) - oldg(0) * ovlp);
        gradxxNum[I] += (g2(0) - oldg2(0) * ovlp);
        grad2Denom[0] += (g2(0) - oldg2(0) * ovlp);
        //grady
        gradyNum[I] += (g(1) - oldg(1) * ovlp);
        gradDenom[1] += (g(1) - oldg(1) * ovlp);
        gradyyNum[I] += (g2(1) - oldg2(1) * ovlp);
        grad2Denom[1] += (g2(1) - oldg2(1) * ovlp);
        //gradz
        gradzNum[I] += (g(2) - oldg(2) * ovlp);
        gradDenom[2] += (g(2) - oldg(2) * ovlp);
        gradzzNum[I] += (g2(2) - oldg2(2) * ovlp);
        grad2Denom[2] += (g2(2) - oldg2(2) * ovlp);
      }
      else
      {
        num[I] += val;
