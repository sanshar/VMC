...
//Iliya style, square of orbital value only for active space
  void asAB2_eval(int elec, const vector<Vector3d> &x, VectorXd &values)
  {
    int norbs = schd.gBasis->getNorbs();
    int nao = schd.asAO.size();

    vector<double> aoValues(10*norbs, 0.0);
    schd.gBasis->eval(x[elec], &aoValues[0]);

    VectorXd num = VectorXd::Zero(nao);
    double denom = 0.0;
    for (int i = 0; i < nao; i++)
    {
        int I = schd.asAO[i];
  ...
  void asAB2_eval_deriv(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad)
  {
    int norbs = schd.gBasis->getNorbs();
    int nao = schd.asAO.size();
    ...
    VectorXd num = VectorXd::Zero(nao);
    VectorXd gradxNum = VectorXd::Zero(nao);
    VectorXd gradyNum = VectorXd::Zero(nao);
    VectorXd gradzNum = VectorXd::Zero(nao);
    double denom = 0.0;
    Vector3d gradDenom = Vector3d::Zero(3);
    for (int i = 0; i < nao; i++)
    {
  ...
  void asAB2_eval_deriv2(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad, array<VectorXd, 3> &grad2)
  {
    int norbs = schd.gBasis->getNorbs();
    int nao = schd.asAO.size();
    ...
    VectorXd gradyNum = VectorXd::Zero(nao);
    VectorXd gradzNum = VectorXd::Zero(nao);
    VectorXd gradxxNum = VectorXd::Zero(nao);
    VectorXd gradyyNum = VectorXd::Zero(nao);
    VectorXd gradzzNum = VectorXd::Zero(nao);
    double denom = 0.0;
    Vector3d gradDenom = Vector3d::Zero(3);
    Vector3d grad2Denom = Vector3d::Zero(3);
    for (int i = 0; i < nao; i++)
...
//s and p orbital AB2 style
  void spAB2_eval(int elec, const vector<Vector3d> &x, VectorXd &values)
  {
    vector<double>& Ncharge = schd.Ncharge;
    vector<int>& Nbasis = schd.Nbasis;
    vector<vector<int>>& NSbasis = schd.NSbasis;
    vector<vector<int>>& NPbasis = schd.NPbasis;
    vector<Vector3d>& Ncoords = schd.Ncoords;
    ...
    for (int i = 0; i < NSbasis.size(); i++) { numSorb += NSbasis[i].size(); }
    int numPorb = 0;
    for (int i = 0; i < NPbasis.size(); i++) { numPorb += NPbasis[i].size(); }

    VectorXd num = VectorXd::Zero(numSorb + numPorb);
    double denom = 0.0;
    ...
    {
        for (int mu = 0; mu < NPbasis[I].size(); mu++)
        {
            int orb = NPbasis[I][mu];
  ...
  void spAB2_eval_deriv(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad)
  {
    vector<double>& Ncharge = schd.Ncharge;
    vector<int>& Nbasis = schd.Nbasis;
    vector<vector<int>>& NSbasis = schd.NSbasis;
    ...
    VectorXd num = VectorXd::Zero(numSorb + numPorb);
    VectorXd gradxNum = VectorXd::Zero(numSorb + numPorb);
    VectorXd gradyNum = VectorXd::Zero(numSorb + numPorb);
    VectorXd gradzNum = VectorXd::Zero(numSorb + numPorb);
    double denom = 0.0;
    Vector3d gradDenom = Vector3d::Zero(3);
  ...
  void spAB2_eval_deriv2(int elec, const vector<Vector3d> &x, VectorXd &values, array<VectorXd, 3> &grad, array<VectorXd, 3> &grad2)
  {
    vector<double>& Ncharge = schd.Ncharge;
    vector<int>& Nbasis = schd.Nbasis;
    vector<vector<int>>& NSbasis = schd.NSbasis;
    ...
    VectorXd gradyNum = VectorXd::Zero(numSorb + numPorb);
    VectorXd gradzNum = VectorXd::Zero(numSorb + numPorb);
    VectorXd gradxxNum = VectorXd::Zero(numSorb + numPorb);
    VectorXd gradyyNum = VectorXd::Zero(numSorb + numPorb);
    VectorXd gradzzNum = VectorXd::Zero(numSorb + numPorb);
    double denom = 0.0;
    Vector3d gradDenom = Vector3d::Zero(3);
    Vector3d grad2Denom = Vector3d::Zero(3);
