...
//-----------------------------------------------------------------------------
    /*!
    Read FCIDUMP file and populate "I1SOC, I2, coreE, nelec, norbs, irrep"
...
//-----------------------------------------------------------------------------
  ...
  if (commrank == 0) {
    ...
    I1SOC.store.clear();
    I1SOC.store.resize(2*norbs*(2*norbs),0.0);
    I1SOC.norbs = 2*norbs;
    coreE = 0.0;
    ...
    while(!dump.eof()) {
      ...
      if(a==b&&b==c&&c==d&&d==0) {
        ...
        I1SOC(2*(a-1),2*(b-1)) = integral; //alpha,alpha
        I1SOC(2*(a-1)+1,2*(b-1)+1) = integral; //beta,beta
        I1SOC(2*(b-1),2*(a-1)) = integral; //alpha,alpha
        I1SOC(2*(b-1)+1,2*(a-1)+1) = integral; //beta,beta
      } else {
        I2(2*(a-1),2*(b-1),2*(c-1),2*(d-1)) = integral;
      }
    } // while
 ...
 I2hb.constructClass(norbs, I2HB);
  ...
  relDeterminant::EffDetLen = (norbs) / 64 + 1;
  relDeterminant::norbs = norbs;
