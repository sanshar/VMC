...
#ifndef relJastrow_HEADER_H
#define relJastrow_HEADER_H
...
class relJastrow {
 ...
 public:
  ...
  double OverlapRatio(const relDeterminant& d1, const relDeterminant& d2) const ;
  /*
   * return ratio of overlaps of Jastrow with d and (i->a,j->b)excited-d (=dcopy)
   */
  double OverlapRatio(int i, int a, const relDeterminant &dcopy, const relDeterminant &d) const ;
  double OverlapRatio(int i, int j, int a, int b, const relDeterminant &dcopy, const relDeterminant &d) const;
   ...
   */
  void   OverlapWithGradient  (const relDeterminant& d, 
                               Eigen::VectorXd& grad,
