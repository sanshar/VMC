#ifndef exCTMC_HEADER_H
#define exCTMC_HEADER_H
#include <Eigen/Dense>
#include <vector>
...
class relContinuousTimeExcitedStates
{
  public:
  Wfn &w;
  Walker &walk;
  ...
  std::complex<double> ovlp, ovlpCurr, Eloc, oldEnergy; // Tc, cumTc;
  double S1;
  ...
  relContinuousTimeExcitedStates(Wfn &_w, Walker &_walk, int niter) : w(_w), walk(_walk)
  {
  ...
  void LocalOverlap(std::vector<Wfn> &ls)
  {
    ovlpCurr = 0.0 + 0.0i;
    w.initWalker(walk, walk.d);
    w.OvlpPenalty(walk, ovlpCurr);
    //cout << "Det " << D << "  ovlp  " << ovlp << "  ovlpLS  ";
    for (int i=0; i<schd.excitedState; i++) {  
      ovlpLS[i] = 0.0 + 0.0i;
      ls[i].initWalker(walk, walk.d);
  ...
  {
    for (int i=0; i<schd.excitedState; i++) {  
      ovlpPen[i] += T * ((ovlpLS[i] / ovlpCurr) - ovlpPen[i]) /cumT;
    }
  }
  ...
  void UpdateGradientPenalty(std::vector<Eigen::VectorXd> &gradPen, const std::vector<std::complex<double>> &ovlpPen)
  {
    for (int i=0; i<schd.excitedState; i++) {
      ...
      gradPen[i] += T* (sgn* grad_ratio  * std::abs(ovlpPen[i]) - gradPen[i]) / cumT;
    }
  }
  ...
  void FinishGradientPenalty(std::vector<Eigen::VectorXd> &gradPen, const Eigen::VectorXd &grad_ratio_bar, const std::vector<std::complex<double>> &ovlpPen)
  {
    for (int i=0; i<schd.excitedState; i++) {
#ifndef SERIAL
      MPI_Allreduce(MPI_IN_PLACE, (gradPen[i].data()), grad_ratio_bar.rows(), MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
#endif
      gradPen[i] /= commsize;
      gradPen[i] = (gradPen[i] - 2*std::abs(ovlpPen[i]) * grad_ratio_bar);
...
/*
template<typename Wfn, typename Walker>
class relCorrelatedSamplingContinuousTime
{
  public:
  int nWave;
  std::vector<Wfn> Wave;
